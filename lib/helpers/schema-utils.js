...
function pathMatcher(routes, path, method) {
  return Object
    ...
    .sort((firstEl, secondEl) => {
      const firstResult = firstEl.split(':').map((value, index) => `${index}`).join('');
      const secondResult = secondEl.split(':').map((value, index) => `${index}`).join('');
    ...
    }).filter(((route) => routes[route][method]))[0];
}
